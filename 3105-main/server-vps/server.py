...
BAN_REASON_DEFAULT = "Key đã bị admin khoá do vi phạm quy định."
...
def api_lock():
    ...
    conn = db()
    conn.execute("UPDATE keys SET locked=1 WHERE key=?", (key,))
    conn.commit()
    conn.close()
    return jsonify(ok=True)
...
def api_seller_create_keys():
    ...
    conn.close()
    return jsonify(ok=True, keys=created, count=len(created), remaining=max(0, remaining - qty))
...
async function createKeys(){
    ...
    try{
        ...
        if(!result.ok){ output.value = result.error || "Không tạo được key"; return; }
        output.value = (result.keys || []).join("\n");
        document.getElementById("created_qty_value").textContent = result.count ?? (result.keys||[]).length;
        await loadAll();
    } catch(error){ output.value = "Lỗi: "+error.message; }
}
