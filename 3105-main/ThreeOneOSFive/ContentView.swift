    ...
    private var brandHeader: some View {
        VStack(alignment: .leading, spacing: 7) {
            ...
            HStack(spacing: 7) {
                ...
                Text(ExploitSupportPolicy.isCurrentOSSupported
                     ? "Thiết bị được hỗ trợ · iOS \(AppInfo.osVersion)"
                     : "Thiết bị không được hỗ trợ · iOS \(AppInfo.osVersion)")
        ...
        HStack(spacing: 11) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text(state.remainingText(from: state.licenseExpiryText))
                    ...
                    .foregroundStyle(state.licenseExpiryText == nil ? .secondary : .green)
            }
        ...
        Task { @MainActor in
            ...
            if feature.available {
                if value {
                    ...
                    state.showToast("Đã bật chức năng tên \(feature.name) thành công!")
                } else {
                    enabled.remove(index)
                    state.showToast("Đã tắt chức năng tên \(feature.name) thành công!")
                }
            } else {
                enabled.remove(index)
