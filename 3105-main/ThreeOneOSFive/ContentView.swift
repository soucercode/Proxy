    ...
    func activate(_ key: String) async {
        ...
        do {
            ...
            guard result.ok else {
                ...
                if message.lowercased().contains("không tồn tại") {
                    showToast("⚠️ Key không tồn tại")
                } else if message.lowercased().contains("thiết bị khác") {
                    showToast("⚠️ Key đã sử dụng cho thiết bị khác")
                } else {
                    showToast("⚠️ \(message)")
                }
                return
                ...
                ScrollView(showsIndicators: false) {
                    VStack(spacing: 16) {
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            Text("Dán key để kích hoạt ứng dụng.")
