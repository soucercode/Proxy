...
struct GameDemoView: View {
    ...
    private let proxyFeatures: [DemoFeature] = [
        DemoFeature(name: "Proxy Aim Body", patchResource: nil),
        DemoFeature(name: "Proxy Aim Neck V2", patchResource: nil),
        DemoFeature(name: "Proxy Aim Neck V1", patchResource: nil),
        DemoFeature(name: "Proxy Aim Drag", patchResource: nil),
        DemoFeature(name: "Magic V4", patchResource: "Magic.3105")
    ]

    private let locationFeatures: [DemoFeature] = [
        DemoFeature(name: "Định Vị Súng Xanh", patchResource: nil),
        DemoFeature(name: "Định Vị Súng Đỏ", patchResource: nil),
        DemoFeature(name: "Định Vị Súng Hồng", patchResource: nil)
    ]
    ...
    private func featureRow(_ feature: DemoFeature, index: Int) -> some View {
        ...
        let usable = state.canUseFeatures
        let canToggle = usable && feature.hasBundledPatch

        return HStack(spacing: 12) {
            Image(systemName: feature.hasBundledPatch ? "bolt.fill" : "wrench.and.screwdriver.fill")
                .foregroundStyle(feature.hasBundledPatch ? accent : .orange)
                .frame(width: 40, height: 40)
                .background(
                    (feature.hasBundledPatch ? accent : Color.orange).opacity(0.14),
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                Text(feature.hasBundledPatch
                     ? "Patch tích hợp sẵn"
                     : "Chức năng đang bảo trì")
                    .font(.caption2)
                    .foregroundStyle(feature.hasBundledPatch ? Color.secondary : Color.orange)
            }
    ...
    private func handleFeature(_ feature: DemoFeature, index: Int, value: Bool) {
        ...
        guard feature.hasBundledPatch, let resourceName = feature.patchResource else {
            enabled.remove(index)
            state.showToast("⚠️ Chức năng đang được bảo trì")
            return
        }
        ...
        Task { @MainActor in
            ...
            do {
                if value {
                    _ = try DevicePatchService.applyBundledDemoPatch(resourceName: resourceName, featureName: feature.name)
