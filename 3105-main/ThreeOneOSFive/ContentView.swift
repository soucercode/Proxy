...
struct GameDemoView: View {
    ...
    private let proxyFeatures: [DemoFeature] = [
        DemoFeature(name: "Proxy Aim Body", patchResource: nil, description: "Full đỏ toàn thân"),
        DemoFeature(name: "Proxy Aim Neck V2", patchResource: nil, description: "HeadShot ngực và cổ siêu bá"),
        DemoFeature(name: "Proxy Aim Neck V1", patchResource: nil, description: "Headshot cổ ít lộ"),
        DemoFeature(name: "Proxy Aim Drag", patchResource: nil, description: "Lộ đỉnh đầu, kéo nhẹ là full đỏ"),
        DemoFeature(name: "Magic V4", patchResource: "Magic.3105", description: "Bắn xung quay người vẫn tính dame")
    ]
    ...
    private func featureRow(_ feature: DemoFeature, index: Int) -> some View {
        ...
        return HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                Text(failedIndices.contains(index)
    ...
    private func handleFeature(_ feature: DemoFeature, index: Int, value: Bool) {
        ...
        // Do not show maintenance before an actual apply attempt. If no bundled
        // payload exists, the operation will fail and the row then changes to the
        // maintenance/X state.
        let resourceName = feature.patchResource
        busyIndex = index
        ...
        Task { @MainActor in
            ...
            do {
                if value {
                    ...
                    // Small delay keeps the loading state visible while the bundled
                    // payload is copied and verified by the app-local demo service.
